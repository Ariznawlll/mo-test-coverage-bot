# `/auto-test-pr` 使用与实现说明

`/auto-test-pr` 是 MatrixOne PR 测试覆盖补充入口。维护者在 `matrixorigin/matrixone` 的 PR 评论区输入该命令后，bot 会分析 PR diff 和 `docs/ai-skills` 知识库，判断 6 类测试是否缺失，并为缺口自动生成跨仓 PR。
