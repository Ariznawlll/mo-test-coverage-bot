...
Legacy one-shot commands `/analyze-pr` and `/gen-chaos-pr` are kept in the script directory for local debugging, but the matrixone bridge only forwards `/auto-test-pr`. Do not advertise the legacy debug commands in user-facing output.

See [docs/auto-test-pr.md](docs/auto-test-pr.md) for the full usage and implementation notes.
...
| `BVT_MO_USER` | `mo_bvt_bot` | Dedicated low-privilege user for BVT result generation. May also be configured as a secret. |
...
| `BVT_PROTECTED_DATABASES` | built-in production/system list | Comma-separated database names that generated BVT SQL must never `USE`, reference explicitly, or affect through database/schema-level DDL. |
