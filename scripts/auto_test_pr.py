...
- SQL 必须是**自闭环**的：允许 `CREATE TABLE` → `INSERT`/DML → `SELECT` 验证 → `DROP TABLE` 清理，避免污染其他 case
- 所有临时表名必须带有 `bvt_<test_name>_` 前缀，避免和已有表重名
- 只使用当前 mo-tester case database 下的非限定表名；不要 `USE <db>`，不要 `CREATE/DROP/ALTER DATABASE/SCHEMA`
...
## 约束
- test_name 只能用小写字母、数字、下划线（3-60 字符）
- SQL 必须完整可执行，包含必要的 `CREATE TABLE` / `DROP TABLE` 清理；只要表名符合前缀规则，表级 DDL 是允许的
...
def _validate_bvt_sql_safety(sql: str, test_name: str) -> None:
    ...
    db_stmt = re.search(
        r"\b(?:use|create(?:\s+or\s+replace)?\s+(?:database|schema)|"
        r"drop\s+(?:database|schema)|alter\s+(?:database|schema))\b",
        clean,
        flags=re.IGNORECASE,
    )
    if db_stmt:
...
def _write_mo_tester_config(tester_dir: str, database: str) -> None:
    ...
    user = os.environ.get("BVT_MO_USER", "mo_bvt_bot").strip()
